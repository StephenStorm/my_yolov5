...
def train(hyp, opt, device, tb_writer=None):
    ...
    if pretrained:
        ...
        if epochs < start_epoch:
            # 如果新设置的epochs 小于加载的epoch
            # 则视新设置的epochs为需要在训练的轮次数而不是总的轮次数
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # Warmup
            # 热身训练（前nw次迭代），在前nw次迭代中，根据一下方式选取accumulate和学习率
            if ni <= nw:
                xi = [0, nw]  # x interp
            ...
            with amp.autocast(enabled=cuda):
                ...
                # loss 为总损失值， loss_item 为一个元祖，包含分类损失，objectness损失，框的回归损失和总损失。
            ...
            # Print
            if rank in [-1, 0]:
                # 打印显存，进行的轮次，损失，target的数量 和图片的size等信息。
        ...
        if rank in [-1, 0]:
            ...
            # 如果设置opt.bucket， 上传resultes.txt到谷歌云盘。
...
if __name__ == '__main__':
    ...
    if opt.global_rank in [-1, 0]:
        # check whether your code version is latest; 
        check_git_status()
        check_requirements()
    ...
    if opt.resume and not wandb_run:  # resume an interrupted run
        ...
        apriori = opt.global_rank, opt.local_rank       # tuple
    ...
    if opt.local_rank != -1:
        ...
        #select gpu according  to gpu-id
        torch.cuda.set_device(opt.local_rank)
        device = torch.device('cuda', opt.local_rank)
        # init process(jin_cheng) group
        dist.init_process_group(backend='nccl', init_method='env://')  # distributed backend
        assert opt.batch_size % opt.world_size == 0, '--batch-size must be multiple of CUDA device count'
        # assign total batch_size to each gpu
    ...
    # Evolve hyperparameters (optional)
        ...
        for _ in range(300):  # generations to evolve
            if Path('evolve.txt').exists():  # if evolve.txt exists: select best hyps and mutate
                ...
                #将突变添加到base hyp上，
                #[i+7]是因为x中前7个数字为results的指标(P, R, mAP, F1, test_losses = (GIOU, obj, cls))， 之后才是超参数
            ...
            '''
            写入results和对应的hyp到evole.txt
            evole.txt文件每一行为一次进化的结果
            一行中前7个数字为(P, R, mAP, F1, test_losses = (GIOU, obj, cls)),之后为hyp
            保存hyp到yaml文件中
            '''
