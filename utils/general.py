...
from utils.diou2 import Weighted_cluster_nms
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # 此时的x是删除掉置信度 < 置信度阈值的结果。也就是筛选后的结果，置信度小于给定阈值的行被删除。
...
def non_max_suppression_cluster_diou(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
                        labels=()):
    """Runs Non-Maximum Suppression (NMS) on inference results
        prediction: batch * n * (5 + nc)
    Returns:
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        boxes_t = boxes.reshape(scores.shape[0], 4)
        # print(boxes_t.shape)
        
        
        # i = torchvision.ops.nms(boxes, scores, iou_thres)  # NMS
        i = Weighted_cluster_nms(boxes_t, scores, iou_thres)
